...
ImportDocsExternesThread::ImportDocsExternesThread()
{
    ...
    m_encours           = false;
    m_nomfichierini     = PATH_FILE_INI;
    m_settings          = new QSettings(m_nomfichierini, QSettings::IniFormat);
    thread              ->start();
}

void ImportDocsExternesThread::RapatrieDocumentsThread(AppareilImagerie *appareil, QString nomfiledoc)     // INCORPORATION DES FICHIERS IMAGE DANS LA BASE  =====
{
    if (m_encours)
        return;
    ...
    else if (Appareil == "EIDON")
    {
        ...
        QRegularExpression re("[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}_[0-9]{2}_[0-9]{2}Z-"); // correspond aux sections 2,3,et 4 avant 2019 eet 3,4 et 5 après
