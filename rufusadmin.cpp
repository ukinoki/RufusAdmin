...
RufusAdmin::RufusAdmin(QWidget *parent) : QMainWindow(parent), ui(new Ui::RufusAdmin)
{
    ...
    qApp->setApplicationVersion("06-09-2018/1");       // doit impérativement être composé de date version / n°version);
...
void RufusAdmin::ConnexionBase()
{
    ...
    if (DlgParam->exec()>0)
        db = DlgParam->getdatabase();
    else
        exit(0);
    if (DlgParam->gMode == dlg_paramconnexion::Poste)
        gMode = Poste;
    else if (DlgParam->gMode == dlg_paramconnexion::ReseauLocal)
        gMode = ReseauLocal;
    else if (DlgParam->gMode == dlg_paramconnexion::Distant)
        gMode = Distant;
}
...
void RufusAdmin::Slot_MetAJourLaConnexion()
{
    ...
    if (verifoldquery.size() > 0)
    {
        ...
        for (int i=0; i<verifoldquery.size();i++)
        {
            ...
            QSqlQuery(req,db);
            req = "SELECT MAX(NewidModifSalDat) From " NOM_TABLE_USERSCONNECTES;
